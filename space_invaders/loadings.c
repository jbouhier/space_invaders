...
t_game    showBegin(t_game game)
{
    ...
    game.begin.instruction_position = init_position(game.begin.high_score_position.x, game.begin.high_score_position.y + 50,
                                                    game.begin.high_score_position.h, game.begin.high_score_position.w);
    game.begin.quit_position = init_position(game.begin.instruction_position.x, game.begin.instruction_position.y + 50,
                                             game.begin.instruction_position.h, game.begin.instruction_position.w);
...
int     checkBeginTexture(t_begin beginGame) {
    ...
    if (beginGame.instruction == NULL)
        printf("the instructions Text's Texture is NULL. Will not render the game\n");
    ...
    if (beginGame.high_score != NULL && beginGame.logo != NULL && beginGame.quit != NULL &&
        beginGame.play_with_1 != NULL && beginGame.selected_option != NULL && beginGame.instruction != NULL) {
        return 1;
    }
    return 0;
}
    ...
    while (SDL_WaitEvent(&game.Gevenements)) {
        ...
        if(game.Gevenements.type == SDL_MOUSEBUTTONDOWN && game.Gevenements.button.button == SDL_BUTTON_LEFT) {
            ...
            if( game.Gevenements.button.x >= game.begin.instruction_position.x &&
               game.Gevenements.button.x <= game.begin.instruction_position.x + game.begin.instruction_position.w &&
               game.Gevenements.button.y >= game.begin.instruction_position.y &&
               game.Gevenements.button.y <= game.begin.instruction_position.y + game.begin.instruction_position.h)
            {
                game.begin.state = 2;
            }
...
t_game  KeyBeginHandler(t_game game) {
    if( game.Gevenements.key.keysym.sym == SDLK_UP) {
        if ( game.begin.selected_option_position.y == game.begin.high_score_position.y + 10)
        ...
        else if (game.begin.selected_option_position.y == game.begin.instruction_position.y + 10)
            ...
            game.begin.selected_option_position.y = game.begin.instruction_position.y + 10;
    }
...
t_game  selectionBeginHandler(t_game game) {
    ...
    if( game.Gevenements.motion.x >= game.begin.play_with_1_position.x &&
       game.Gevenements.motion.x <= game.begin.play_with_1_position.x + game.begin.play_with_1_position.w) {
        ...
        else if (game.Gevenements.motion.y >= game.begin.instruction_position.y &&
                 game.Gevenements.motion.y <= game.begin.instruction_position.y + game.begin.instruction_position.h) {
            game.begin.selected_option_position.y = game.begin.instruction_position.y + 10;
            cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_HAND);
        }
        else if (game.Gevenements.motion.y >= game.begin.quit_position.y &&
                 game.Gevenements.motion.y <= game.begin.quit_position.y + game.begin.quit_position.h) {
            game.begin.selected_option_position.y = game.begin.quit_position.y + 10;
            cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_HAND);
        }
    }
